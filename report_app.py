...
WRITING_DATA = {
    ...
    "Training for Reading S1": ["Vocabulary", "Training 1~11", "스피킹&라이팅"],
    "Training for Reading S2": ["Vocabulary", "Training 1~11", "스피킹&라이팅"],
    "Training for Reading S3": ["Vocabulary", "Training 1~9", "Story", "스피킹&라이팅"],
    "Training for Reading S4": ["Vocabulary", "Training 1~9", "Story", "스피킹&라이팅"]
}
...
if st.session_state.page == 'input':
    ...
    with st.sidebar:
        ...
        if st.session_state.saved_reports:
            st.write(f"현재 {len(st.session_state.saved_reports)}개의 리포트가 대기 중")
            
            # 엑셀 다운로드 로직
            report_df = pd.DataFrame(st.session_state.saved_reports)
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                report_df.to_excel(writer, index=False, sheet_name='Daily_Reports')
            st.download_button(label="📥 전체 리포트 엑셀 다운로드", data=output.getvalue(), 
                               file_name=f"LK_Reports_{datetime.now().strftime('%Y%m%d')}.xlsx",
                               mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            
            if st.button("🗑️ 저장 목록 초기화", type="secondary"):
                st.session_state.saved_reports = []; st.rerun()
        else:
    ...
    grade = c1.selectbox("학년", ["초등", "중등", "고등"], index=find_index(["초등", "중등", "고등"], get_safe("학년")))
    ...
    p_hw = st.checkbox("과제X (파닉스)", key="p_hw")
    ...
    elt_hw = st.checkbox("과제X (ELT)", key="elt_hw")

    ns_options = ["선택 안 함", "<Kinder>", "<Kids>"]
    news_paper = st.selectbox("영자신문 선택", ns_options, index=find_index(ns_options, get_safe("영자신문")))
    news_unit = st.text_input("└ 영자신문 주제 입력", placeholder="예: <Unit 01>")
    news_hw = st.checkbox("과제X (영자신문)", key="news_hw")
    ...
    r_hw = st.checkbox("과제X (독해)", key="r_hw")
    ...
    w_hw = st.checkbox("과제X (라이팅)", key="w_hw")
    ...
    if st.button("🤖 과제 분석 시작") and up_file and domain != "선택 안 함":
        with st.spinner("분석 중..."):
            try:
                ...
                res = client.models.generate_content(model="gemini-1.5-flash", contents=[f"성취도 분석", img])
    ...
    if st.button("리포트 생성", type="primary"):
        if not name: st.warning("이름을 입력하세요.")
            ...
            if elt_book != "선택 안 함": items.append(f"• ELT독해: {elt_book} [{elt_unit}]" + (" (과제X)" if elt_hw else ""))
            if news_paper != "선택 안 함": items.append(f"• 영자신문: {news_paper} [{news_unit}]" + (" (과제X)" if news_hw else ""))
            if r_book != "선택 안 함": items.append(f"• 독해: {r_book} [{r_unit}]" + (" (과제X)" if r_hw else ""))
            ...
            st.session_state.temp_data = {
                "날짜": report_date, "학생명": name, "학년": grade, "반": class_name,
                "어휘교재": v_book if v_book != "선택 안 함" else "", "어휘결과": ", ".join(v_results) if v_book != "선택 안 함" else "",
                "학습내용": "\n".join(items), "피드백": content, "과제분석": ai_fb, "과제수행도": hw_status
            }
    ...
    with col1:
        if st.button("✅ 리포트 목록에 누적 저장", type="primary"):
            st.session_state.saved_reports.append(st.session_state.temp_data)
            st.success("목록에 저장되었습니다! 왼쪽 사이드바에서 엑셀로 받을 수 있습니다.")
    with col2:
        if st.button("↩️ 저장 없이 돌아가기"):
