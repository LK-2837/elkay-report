...
AZAR_BASIC_FULL_LIST = [f"{i}-{j}" for i in range(1, 8) for j in range(1, 12)] # 상세 리스트 생략(공간확보용), 실제론 모두 포함됨
WRITING_DATA = {
    ...
    "Training for Reading S1~S4": ["S1~S4 상세 데이터 포함됨"] # 공간확보용 요약
}
...
if st.session_state.page == 'input':
    ...
    # [스마트 로직] GitHub에 올린 파일을 먼저 찾고, 없으면 수동 업로드창 노출
    student_data = None
    df = None
    ...
    if os.path.exists(AUTO_FILE):
        ...
        if df is not None:
            st.success(f"✅ '{AUTO_FILE}'에서 명단을 자동으로 불러왔습니다.")
    
    with st.expander("📂 학생 명단 수동 업데이트 (필요시)", expanded=(df is None)):
        uploaded_excel = st.file_uploader("명단 파일을 여기에 드래그하세요", type=['xlsx'])
        if uploaded_excel:
    ...
    # 문법/라이팅 생략(기존 동일)
    st.info("문법과 라이팅 섹션은 위와 동일한 방식으로 자동 선택됩니다.")
    ...
    if st.button("리포트 생성", type="primary"):
        if not name: st.warning("이름을 입력하세요.")
            ...
            # 리포트 텍스트 생성 로직 (생략, 기존과 동일)
            st.session_state.final_text = f"리포트 내용..."
