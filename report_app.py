...
if st.session_state.page == 'input':
    ...
    v_unit = cv1.text_input("Unit", placeholder="1")
    ...
    g_book = st.selectbox("Grammar (문법)", ["선택 안 함", "Azar Basic", "Azar Fundamentals", "기타"])
    r_book = st.selectbox("Reading (독해/뉴스)", ["선택 안 함"] + READING_BOOKS_DATA)
    w_book = st.selectbox("Writing (라이팅 시리즈)", ["선택 안 함"] + list(WRITING_DATA.keys()))
    
    # 라이팅 선택 시 세부 단원 자동 노출
    w_ls = ""
    if w_book != "선택 안 함":
    ...
    if uploaded_file and domain != "선택 안 함":
        st.success(f"✅ {domain} 과제 사진이 확인되었습니다.")
        if st.button("🤖 AI 과제 분석 시작"):
            with st.spinner("AI 선생님이 분석 중입니다..."):
                try:
                    img = Image.open(uploaded_file)
                    img.thumbnail((1024, 1024))
                    prompt = f"너는 엘케이어학원 선생님이야. 이 {domain} 과제 사진을 보고 학생의 학습 상태를 친절하게 분석해줘. 한국어로 2~3문장으로 써줘."
                    response = client.models.generate_content(model="gemini-1.5-flash", contents=[prompt, img])
                    st.session_state.ai_res = response.text
                except Exception as e:
                    st.error(f"AI 오류: {e}")

    ai_feedback = st.text_area("AI 분석 결과 확인", value=st.session_state.ai_res, height=150)

    st.divider()

    # 5. 피드백 및 수행도 섹션
    st.subheader("✍️ 선생님 총평")
    content = st.text_area("선생님 상세 피드백", placeholder="오늘의 학습 태도 및 성취도를 적어주세요.")
