...
from google import genai  # 최신형 구글 AI 라이브러리
...
if st.session_state.page == 'input':
    ...
    g_book = ce2.selectbox("문법 (Grammar)", ["선택 안 함", "Azar 시리즈", "자체 교재", "기타"])
        ...
        if st.button("🤖 AI 과제 분석 시작"):
            with st.spinner("AI 선생님이 분석 중입니다..."):
                try:
                    ...
                    # 모델 식별자 형식을 가장 표준적인 형태로 수정 (404 오류 해결)
    ...
    # [섹션 5] 선생님 총평 (복구 완료)
    st.header("5. 선생님 총평")
    content = st.text_area("선생님 상세 피드백", placeholder="오늘의 수업 내용과 태도를 입력하세요.")
